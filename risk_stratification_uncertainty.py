...
def prs_learning(train_X, train_y, val_X, val_y, test_X, test_y):
    colname = ['SCOREp1e5']
    feature_names = colname + ['APOE_e2e4']
    clf = LogisticRegression(solver='newton-cg', random_state=0)
    ...
    performance_basedon_uncertainty(clf, val_X, val_y, feature_names, 'prs', cutoff)
    performance_basedon_uncertainty(clf, test_X, test_y, feature_names, 'prs', cutoff)
    return clf, cutoff
...
def learn_and_uestimate(train_X, train_y, val_X, val_y, test_X, test_y, feature_names, labelname):
...
def single_run(train_X, train_y, val_X, val_y, test_X, test_y):
    adors_clf, adors_cutoff = adors_learning(train_X, train_y, val_X, val_y, test_X, test_y)
    prs_clf, prs_cutoff = prs_learning(train_X, train_y, val_X, val_y, test_X, test_y)
    all_clf, all_cutoff = learn_and_uestimate(train_X, train_y, val_X, val_y, test_X, test_y, config.feature_names_ts, 'all')
    integrated_clf, integrated_cutoff = learn_and_uestimate(train_X, train_y, val_X, val_y, test_X, test_y, config.feature_names_ts + genelist, 'integrated')

    val_X['y'] = val_y
    test_X['y'] = test_y

    clf = {'adors_clf': adors_clf, 'prs_clf': prs_clf, 'all_clf': all_clf, 'integrated_clf': integrated_clf}
    cutoff = {'adors_cutoff': adors_cutoff, 'prs_cutoff': prs_cutoff, 'all_cutoff': all_cutoff, 'integrated_cutoff': integrated_cutoff}
    val = {'val_X': val_X[['RID','adors_pred', 'adors_cut','all_pred','all_cut','prs_pred','prs_cut','integrated_pred','integrated_cut','y']], 'val_y': val_y}
    test = {'test_X': test_X[['RID','adors_pred', 'adors_cut','prs_pred','prs_cut','all_pred','all_cut','integrated_pred','integrated_cut','y']], 'test_y': test_y}
    return clf, cutoff, val, test
...
def main(X, y, rm):
    ...
    for i in range(n_iter):
        ...
        for dev_index, test_index in skf.split(X,y):
            dev_X, dev_y = X.iloc[dev_index, :], y.iloc[dev_index]
            test_X, test_y = X.iloc[test_index, :], y.iloc[test_index]
            sss = StratifiedShuffleSplit(n_splits=1, test_size=0.20)

            train_index, val_index = list(sss.split(dev_X, dev_y))[0]
            train_X, train_y = dev_X.iloc[train_index], dev_y.iloc[train_index]
            val_X, val_y = dev_X.iloc[val_index], dev_y.iloc[val_index]
            clf, cutoff, val, test = single_run(train_X, train_y, val_X, val_y, test_X, test_y)
